...
const float freq_band_end_hz[freq_band_count] = {30.8f, 47.4f, 73.0f, 112.5f, 173.2f, 266.7f, 410.7f, 632.5f, 973.9f, 1499.8f, 2309.6f, 3556.6f, 5476.8f, 8433.9f, 12987.6f, 20000.0f};
const float freq_band_amp[freq_band_count] = {1.0f, 1.6f, 2.2f, 2.8f, 3.4f, 4.0f, 4.6f, 5.2f, 5.8f, 6.4f, 7.0f, 7.6f, 8.2f, 8.8f, 9.4f, 10.0f};
    ...
    for (uint8_t ii = 0; ii < freq_band_count; ii++) {
        ...
        for (uint8_t jj = 0; jj < height; jj++) {
            for (uint8_t kk = 0; kk < bar_width; kk++) {
                display->drawPixel(ii * bar_width + kk, PANEL_RES_Y - 1 - jj, display->color565((255 * jj) / (PANEL_RES_Y - 1), (255 * (PANEL_RES_Y - 1 - jj)) / (PANEL_RES_Y - 1), 0));
            }
        }
    }
}
